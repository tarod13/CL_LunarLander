...
ExperienceFirstLevel = namedtuple(
    ...
    field_names=['states', 'actions', 'rewards', 'dones', 'dists'])
...
class EpisodeExperienceBuffer:
    def __init__(self, capacity, use_pixels=False, level=1):
    ...
    def sample(
        ...
        if self._level == 1:
            ...
            for epsd_idx in epsd_indices:
                epsd_states, epsd_actions, epsd_rewards, \
                ...
                states = sample_from_deque(
                    epsd_states, step_idx_init, step_idx_fin+1)
                ...
                if not is_long_episode:
                    ...
                    states = states + steps_missing*[states[-1]]
                ...
                sampled_states.append(np.stack(states).astype(
                    np.uint8 if self._use_pixels else np.float32))
            ...
            if to_torch:
                ...
                sampled_states = np.stack(sampled_states).astype(np.float32)
                if self._use_pixels:
                    sampled_states /= 255.
                states_th = torch.FloatTensor(sampled_states).to(device)
                ...
                return states_th, np.stack(sampled_actions).astype('int'), \
                    rewards_th, dones_th, dists_th
            
            else:
                return np.stack(sampled_states), np.stack(sampled_actions), \
